...
import atexit
url='https://ingest.home.egouda.xyz/api/state'
anonymous=urllib.request.build_opener(NoRedirect)
try:
    anonymous.open(url,timeout=10)
    raise SystemExit('FAIL: unauthenticated tracking data was accessible.')
except urllib.error.HTTPError as error:
    assert error.code==302 and error.headers['Location'].startswith('https://auth.home.egouda.xyz/'),'Expected central sign-in redirect'
print('PASS: unauthenticated requests require central sign-in.')
gateway=AuthSession();atexit.register(gateway.close)
headers={'Cookie':gateway.cookie_header(url)}
with gateway.opener.open(url,timeout=10) as response:
...
request=urllib.request.Request('https://ingest.home.egouda.xyz/api/retry',b'{}',
